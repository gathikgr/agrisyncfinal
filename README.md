...
- `components/` reusable UI components (language cards, chart, PWA register, app header)
- `lib/` auth, i18n, db adapter, local cache/session helpers
...
5. Buyer dashboard filters future supply (crop/location/harvest window) and creates pre-harvest commitments.

## Supabase Notes
- Add `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY` in `.env.local` to enable Supabase auth.
- Profile setup and buyer commitments are persisted to Supabase when configured.
- If env vars are missing, app uses local demo storage so the app still runs out of the box.
